...
/**
 * Ambil events dengan server-side pagination yang sesuai respons di atas.
 * Return bentuk konsisten: { items, meta }
 */
...
export async function getProfile(email_crypted) {
  const url = `${API_CONFIG.BASE_URL}/profile/${email_crypted}`;
  try {
    const res = await apiFetch(url, {
      method: "GET",
      headers: { "Content-Type": "application/json" },
